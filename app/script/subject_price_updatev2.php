...
function updateSubjectPrice($db,$purchaseId){
	...
	while($row = $result->fetch_assoc()){
		...
		$original_price =!empty($getSubjectRow['original_price']) ? $getSubjectRow['original_price'] : '';
		$original_price = ($original_price=='999') ? '1399':$original_price;
		$offer_price = !empty($getSubjectRow['offer_price']) ? $getSubjectRow['offer_price'] : '';
		$offer_price = ($offer_price=='599') ? '899':$offer_price;
